...
def createHtmlBody(args): #To add table: create sql file in doctor/sql/ --> html += htmltable( title of the table, filename of sql )
    ...
    html += "</div>" #div end of page (line break)
    html += "src: <p>http://gitlabcntrl.pol.be/dba/postgres_hc</p>"
    return html
...
def addHeadersAndStyle(htmltoadd):
    #logo= "../img/INT_Screen_Verti_F_Color.png"
    ...
    datapath=os.path.join("img", "img.txt") 
    html += "<div class='pages'><img src='data:image/png;base64," + open(datapath,"r").read() + "' style='width:5%'><h1>Postgresql Health Check</h1></div></div></div>"
    html +=  htmltoadd
